      ...
      <div class="modal-content">
        ...
        <form id="player-form">
          ...
          <div class="player-info">
            <label for="player1" style="margin-right: 10px; margin-top: 15px"
              >Player 1 :</label
            >
            ...
            <div class="color-options">
              <div
                ...
                onclick="selectColor('yellow', 'color1')"
              ></div>
            </div>
          </div>
          <div class="player-info">
            <label for="player2" style="margin-right: 10px; margin-top: 15px"
              >Player 2 :</label
            >
            ...
            <div class="color-options">
              <div
                ...
                onclick="selectColor('green', 'color1')"
              ></div>
            </div>
          </div>
          <div class="player-info">
            <label for="player3" style="margin-right: 10px; margin-top: 15px"
              >Player 3 :</label
            >
            ...
            <div class="color-options">
              <div
                ...
                onclick="selectColor('blue', 'color1')"
              ></div>
            </div>
          </div>
          <div class="player-info">
            <label for="player4" style="margin-right: 10px; margin-top: 15px"
              >Player 4 :</label
            >
            ...
            <div class="color-options">
              <div
                ...
                onclick="selectColor('red', 'color1')"
              ></div>
            </div>
          </div>

          <button type="button" id="startButton" @click="startGame">
            Spiel starten
          </button>
        </form>
      </div>
  ...
  methods: {
    ...
    adjustGameBoard() {
      if (this.playeramount < 2 || this.playeramount > 4) {
        console.error(
          "Ungültige Spieleranzahl. Die Spieleranzahl muss zwischen 2 und 4 Spielern liegen."
        );
      }
    ...
    async isMovingPieceOutAllowed(cell) {
      ...
      const startingTile = this.findCellByRowAndColumn(
        startingTileX,
        startingTileY
      );
    ...
    async movePiece(cell) {
      ...
      //await this.sleep(300);
      if (
        cell.classList.contains("housep1") ||
        cell.classList.contains("housep2") ||
        cell.classList.contains("housep3") ||
        cell.classList.contains("housep4")
      ) {
        return false;
      }
      ...
      if (pieceColor === currentPlayerColor) {
        ...
        this.addPlayerCircle(
          this.findCellByRowAndColumn(newPosX, newPosY),
          currentPlayerColor
        );
    ...
    async rollMagicDice() {
      ...
      this.changeTextFieldText(
        "Player " + (this.playerturn + 1) + " rolled a 6. It's your turn again!"
      );
    },
      ...
      if (this.rolledDice === 6 && (await this.isOnePieceOut())) {
        ...
        this.changeTextFieldText(
          "Player " +
            ...
            " rolled a " +
            this.rolledDice +
            ". Choose a Piece to get out or move a Piece on the board. It's your turn again!"
        );
        return;
        ...
        this.changeTextFieldText(
          "Player " +
            ...
            " rolled a " +
            this.rolledDice +
            ". Choose a Piece to get out. It's your turn again!"
        );
        return;
        ...
        this.changeTextFieldText(
          "Player " +
            ...
            " rolled a " +
            this.rolledDice +
            ". Choose the Piece to move. It's Player " +
    ...
    createRow(rowData, rowIndex) {
      ...
      const maxHeight = Math.max(
        ...rowData.map((cellData) => (cellData.className === "empty" ? 0 : 1))
      );
      ...
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getPlayerturn"
          );
    ...
    async getRolledDiceFromBackend() {
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getRolledDice"
          );
    ...
    async getPlayeramountFromBackend() {
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getPlayeramount"
          );
    ...
    async getTimesPlayerRolledFromBackend() {
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getTimesPlayerRolled"
          );
    ...
    async getPiecesOutFromBackend() {
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getPiecesOut"
          );
    ...
    async getPiecesListFromBackend() {
      try {
        if (this.socketActive) {
          const response = await axios.get(
            "http://localhost:9000/getPiecesList"
          );
      ...
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setPlayerturn",
            {
              playerturnBackend,
            }
          );
          ...
          this.playerturn = playerturnBackend;
        }
      } catch (error) {
    ...
    async setRolledDiceInBackend(rolledDiceBackend) {
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setRolledDice",
            {
              rolledDiceBackend,
            }
          );
          ...
          this.rolledDice = rolledDiceBackend;
        }
      } catch (error) {
        console.error("Fehler beim Senden an das Backend:", error);
      }
    ...
    async setPlayeramountInBackend(playeramountBackend) {
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setPlayeramount",
            {
              playeramountBackend,
            }
          );
          ...
          this.playeramount = playeramountBackend;
        }
      } catch (error) {
        console.error("Fehler beim Senden an das Backend:", error);
      }
    ...
    async setTimesPlayerRolledInBackend(timesPlayerRolledBackend) {
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setTimesPlayerRolled",
            {
          ...
          this.timesPlayerRolled = timesPlayerRolledBackend;
        }
      } catch (error) {
        console.error("Fehler beim Senden an das Backend:", error);
      }
    ...
    async setPiecesOutInBackend(pieceOutBackend) {
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setPiecesOut",
            {
              pieceOutBackend,
            }
          );
    ...
    async setPiecesListInBackend(piecesListBackend) {
      try {
        if (this.socketActive) {
          const response = await axios.post(
            "http://localhost:9000/setPiecesList",
            {
              piecesListBackend,
            }
          );
          ...
          this.pieceList = piecesListBackend;
        }
      } catch (error) {
        console.error("Fehler beim Senden an das Backend:", error);
      }
